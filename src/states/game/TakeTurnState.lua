            ...
            Timer.after(1.5, function()
                ...
                :finish(function()
                    ...
                    if self.playerPokemon.currentExp > self.playerPokemon.expToLevel then
                        ...
                        local HP_before = self.playerPokemon.HP
                        local attack_before = self.playerPokemon.attack
                        local defense_before = self.playerPokemon.defense
                        local speed_before = self.playerPokemon.speed

                        self.playerPokemon:levelUp()
                        
                        local HP_after = self.playerPokemon.HP
                        local attack_after = self.playerPokemon.attack
                        local defense_after = self.playerPokemon.defense
                        local speed_after = self.playerPokemon.speed
                        
                        local lvlupmenu = Menu{
                            x = math.floor(VIRTUAL_WIDTH / 6),
                            y = math.floor(VIRTUAL_HEIGHT / 6),
                            width = math.floor(2 * VIRTUAL_WIDTH / 3),
                            height = math.floor(VIRTUAL_HEIGHT / 2),
                            hasCursor = false,
                            items = {
                                {
                                    text = 'HP: ' .. tostring(HP_before) .. ' + ' .. tostring(HP_after - HP_before) .. ' -> ' .. tostring(HP_after),
                                    onSelect = function()
                                        gStateStack:pop() 
                                        self:fadeOutWhite() 
                                    end
                                ...
                                {
                                    text = 'attack: ' .. tostring(attack_before) .. ' + ' .. tostring(attack_after - attack_before) .. ' -> ' .. tostring(attack_after)
                                },
                                {
                                    text = 'defense: ' .. tostring(defense_before) .. ' + ' .. tostring(defense_after - defense_before) .. ' -> ' .. tostring(defense_after)
                                },
                                {
                                    text = 'speed: ' .. tostring(speed_before) .. ' + ' .. tostring(speed_after - speed_before) .. ' -> ' .. tostring(speed_after)
                                }
                            }
                        }
                        ...
                        function()
                            gStateStack:push(MenuState(lvlupmenu))
                        end))
                    else
                        self:fadeOutWhite()
                    end
