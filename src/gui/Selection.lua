...
function Selection:init(def)
    ...
    if self.hasCursor == nil then self.hasCursor = true end
end

function Selection:update(dt)

    if love.keyboard.wasPressed('up') and self.hasCursor then
        ...
        gSounds['blip']:stop()
        gSounds['blip']:play()
    elseif love.keyboard.wasPressed('down') and self.hasCursor then
...
function Selection:render()
    ...
    for i = 1, #self.items do
        ...
        if i == self.currentSelection  and self.hasCursor then
